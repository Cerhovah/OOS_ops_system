...
export default function PlanScreen() {
  ...
  useEffect(() => {
    const values = Object.fromEntries(
      accounts.map((account) => {
        const line = current.lines.find((candidate) => candidate.accountId === account.id);
        return [account.id, line ? String(line.plannedMinutes / 60) : '0'];
      }),
    );
    setHours(values);
  }, [accounts, current]);
  ...
  async function save(source: 'app' | 'copy_last_week' = 'app', note: string | null = null) {
    const values = Object.fromEntries(accounts.map((account) => [account.id, Number(hours[account.id]) * 60]));
    const version = await app.saveWeeklyPlan(weekStart, values, source, note);
  ...
  async function restore(planId: string, version: number) {
    const lines = app.snapshot.planLines.filter((line) => line.weeklyPlanId === planId);
  ...
  const versions = app.snapshot.plans.filter((plan) => plan.weekStart === weekStart);
      ...
      <Section title="계정별 주간 시간">
        {accounts.map((account) => (
          <Field
            ...
            onChangeText={(value) => setHours((currentHours) => ({ ...currentHours, [account.id]: value }))}
            keyboardType="decimal-pad"
          />
        ))}
        <AppButton label="계획 저장" onPress={confirmSave} disabled={!valid || app.busy} />
        <AppButton label="지난주 계획 복사" variant="secondary" onPress={() => void copyPrevious()} />
      </Section>
      <Section title="버전 이력">
        {versions.length === 0 ? <Text style={textStyles.body}>저장된 버전이 없습니다.</Text> : null}
        {versions.map((version) => {
          const total = app.snapshot.planLines
            .filter((line) => line.weeklyPlanId === version.id)
          ...
          return (
            <Card key={version.id}>
              ...
              <AppButton label="이 버전 복원" variant="secondary" onPress={() => void restore(version.id, version.version)} />
            </Card>
          );
        })}
      </Section>
