...
const manifestPath = path.resolve(scriptDir, '../../docs/design/p5-visual-v3.gate.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const featureName = typeof manifest.feature === 'string' ? manifest.feature : 'P5 visual design';
const stageIndex = process.argv.indexOf('--stage');
const stage = stageIndex >= 0 ? process.argv[stageIndex + 1] : 'implementation';

if (stage === 'status') {
  console.log(`${featureName}: ${manifest.status}`);
...
if (!manifest.approvals.productPlan) missing.push('product plan approval');
...
if (!manifest.figma.sourcePageId) missing.push(`${featureName} page id`);
...
if (missing.length > 0) {
  console.error(`${featureName} ${stage} gate is blocked:`);
  for (const item of missing) console.error(`- ${item}`);
  process.exit(1);
}

console.log(`${featureName} ${stage} gate passed.`);
