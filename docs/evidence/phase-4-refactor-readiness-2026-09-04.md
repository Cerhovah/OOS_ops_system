...
| clean Supabase DB | `npx supabase@2.116.0 db start`, container `psql` RLS assertion, `stop --no-backup` | **재검증 중**: migration 적용은 통과. 첫 실행은 기존 Auth 사용자 의존, 두 번째는 multi-statement 파일에 단일 prepared query 경로를 써 실패했다. 자체 임시 사용자+rollback과 컨테이너 `psql`로 수정 후 재실행 대기 |
