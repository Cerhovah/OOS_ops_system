...
2026-09-04 사용자 지시로 Phase 4R 동작 보존 리팩터와 Phase 4S Android 개인용 standalone을 SPEC v0.3.0에 추가했다. 현재 `0.4.1(8)` 리팩터는 전체 자동·linked 원격 hardening 게이트를 통과했지만 인증 실호출·새 native build 실기기·CI가 대기 중이고, PC·Metro 없는 standalone binary도 아직 생성되지 않았다.
...
| Phase 4R | Phase 1·2·4 동작 보존, 구조 분리·보안·재현성 강화 | `0.4.1(8)` 전체 자동·linked 원격 hardening 통과, 인증 실호출·새 build 실기기·CI 대기 |
...
- 현재 development APK: 개발 도구와 launcher가 포함된 내부 테스트용이다. 앱을 삭제하지 않는 한 EAS 다운로드 URL 만료 때문에 자동으로 사라지지는 않지만 JavaScript 실행에는 Metro가 필요하다. 마지막 실기기 native build는 `0.2.0(3)`이고 `0.4.1(8)`은 SecureStore native module 때문에 새 development build 검증이 필요하다.
